...
export default async function DocumentsPage() {
  ...
  return <AppShell navigation={buildProtectedAppNavigation(access)} scrollOwner="main" topBar={<ContextTopBar density="compact" title="Documents" secondary="Document tools"/>}><Workspace width="fluid"><section className="grid gap-2 md:grid-cols-2">{DOORGO_DOCUMENT_DEFINITIONS.map((definition) => {
    const permitted = hasAtLeastView(access, definition.permissionKey);
  ...
  })}</section><p className="text-xs text-slate-500">DoorGo does not yet have a persisted document library. Future document types require separately approved contracts.</p></Workspace></AppShell>;
}

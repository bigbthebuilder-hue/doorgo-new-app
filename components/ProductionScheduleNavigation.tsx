...
export function ProductionWindowNavigation({ anchorMonday, currentMonday, pathname, label, onNavigate }: {
  ...
  return <div aria-label={label} className="production-window-navigation flex items-end gap-1">
    ...
    <form className="production-window-date flex items-end gap-1" onSubmit={(event) => { event.preventDefault(); if (isValidDateOnly(selectedDate)) navigate(normalizeProductionWeekAnchor(selectedDate, currentMonday)); }}>

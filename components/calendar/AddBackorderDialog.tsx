...
type SavedJob={linkedInternalJobId:string;baseSalesOrder:string;customer:string};
export function AddBackorderDialog({baseSalesOrder,customer,linkedInternalJobId,onClose,onCreated,beforeCreate}:{baseSalesOrder:string;customer:string;linkedInternalJobId:string;onClose:()=>void;onCreated?:(card:ProductionBoardCard,salesOrder:string)=>void;beforeCreate?:()=>Promise<SavedJob|null>}){
  ...
  const submit=async(event:React.FormEvent)=>{event.preventDefault();setPending(true);setError(null);const saved=beforeCreate?await beforeCreate():{linkedInternalJobId,baseSalesOrder,customer};if(!saved){setPending(false);return;}const result=await addBackorder({commandId:crypto.randomUUID(),linkedInternalJobId:saved.linkedInternalJobId,baseSalesOrder:saved.baseSalesOrder,itemType,scheduledDate:scheduled,timing,fulfillmentNote:note,today:getCurrentDateInTimeZone('America/Vancouver')});setPending(false);if(!result.ok){setError(result.message);return;}onCreated?.(result.card,result.salesOrder);onClose();};

...
export function AddBackorderDialog({baseSalesOrder,customer,linkedInternalJobId,onClose,onCreated}:{baseSalesOrder:string;customer:string;linkedInternalJobId:string;onClose:()=>void;onCreated?:(card:ProductionBoardCard,salesOrder:string)=>void}){
  ...
  const submit=async(event:React.FormEvent)=>{event.preventDefault();if(pending)return;setPending(true);setError(null);try{const result=await addBackorder({commandId:crypto.randomUUID(),linkedInternalJobId,baseSalesOrder,itemType,scheduledDate:scheduled,timing,fulfillmentNote:note,today:getCurrentDateInTimeZone('America/Vancouver')});if(!result.ok){setError(result.message);return;}onCreated?.(result.card,result.salesOrder);onClose();}catch{setError('Backorder fulfillment could not be added. Please try again.');}finally{setPending(false);}};
  return <div className="calendar-floating-backdrop"><form className="calendar-quick-add calendar-quick-add-form calendar-backorder-dialog" onSubmit={submit}><header><strong>Schedule Backorder Fulfillment</strong><button aria-label="Close Backorder Fulfillment" onClick={onClose} type="button">×</button></header><p>Create a Delivery or Customer Pickup for another Sales Order in this order family.</p><p>{customer} · Family {baseSalesOrder}</p>
    ...
    {error?<p role="alert">{error}</p>:null}<footer><button disabled={pending} onClick={onClose} type="button">Cancel</button><button aria-busy={pending||undefined} disabled={pending||nextSalesOrder===null} type="submit">{pending?'Adding…':'Add Backorder Fulfillment'}</button></footer>
  </form></div>;
}

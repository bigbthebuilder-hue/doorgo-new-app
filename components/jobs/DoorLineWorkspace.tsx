...
export function DoorLineWorkspace({ lines, onChange, onUnappliedChange, canEdit, lifecycleStage }: {
  ...
  return <section className="grid min-w-0 gap-2 xl:grid-cols-[minmax(0,1.25fr)_minmax(320px,.75fr)]" aria-labelledby="door-lines-heading">
    <div className="min-w-0 rounded-lg border border-slate-200 bg-white p-2.5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      ...
      {!canEdit ? <p className="mt-4 rounded-xl bg-sky-50 p-3 text-sm text-sky-900 dark:bg-sky-950 dark:text-sky-100">Door lines and geometry are read-only with jobs = view.</p> : <>
        ...
        <div className="mt-2 grid gap-2 sm:grid-cols-3 2xl:grid-cols-4">
        ...
        <div className="mt-2 grid gap-2 border-t border-slate-200 pt-2 sm:grid-cols-3 2xl:grid-cols-4" aria-label="Door production configuration">
          ...
          <label className="grid gap-1 text-sm font-semibold sm:col-span-3 2xl:col-span-4">Line Notes<textarea className={`${control} min-h-14 py-1.5`} onChange={(event) => set('notes', event.target.value)} value={String(editor.notes ?? '')}/></label>
        </div>
        <div className="mt-2 rounded-md bg-slate-100 p-2 text-xs dark:bg-slate-800"><span className="font-semibold">Preview:</span> {lineTitle(editor)}</div>
        <div className="mt-2 flex flex-wrap gap-2"><button className={`${button} border-sky-700 bg-sky-700 text-white`} onClick={() => commitEditor()} type="button">{editingIndex !== null ? 'Update Door' : 'Add Door'}</button>{editingIndex !== null ? <button className={button} onClick={resetEditor} type="button">Cancel Edit</button> : null}</div>
      </>}

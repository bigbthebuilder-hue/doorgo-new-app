...
import { automaticSidelightTBar, automaticTransomTBar, calculateGlassGeometry, glassConfigurationTopology, normalizeGlassTypeCode, normalizeSidelightType, numericDimension } from '@/lib/jobs/glass-geometry-contract';
...
export function GlassUnitBuilder({ line, onCancel, onUse }: {
  ...
  function setUnitSidelightType(value: SidelightType) {
    setDraft((current) => {
      const priorType = normalizeSidelightType(current.sidelightType) ?? 'Glass';
      const specifications = canonicalSidelightSpecifications(current).map((entry) => {
        const retainedTBar = entry.tBarSize && entry.tBarSize !== automaticSidelightTBar(priorType) ? entry.tBarSize : automaticSidelightTBar(value);
        if (value === 'Glass') return { ...entry, tBarSize: retainedTBar, glassTypeCode: entry.glassTypeCode ?? 'CLEAR' as const, panelSizeMode: null, panelConstructionNotes: null };
        const parsed = numericDimension(entry.finishedWidth);
        const standard = parsed.ok && (parsed.inches === 11.75 || parsed.inches === 13.75);
        return { ...entry, finishedWidth: parsed.ok ? entry.finishedWidth : '11 3/4', tBarSize: retainedTBar, glassTypeCode: null, customGlassDescription: null, panelSizeMode: standard ? 'standard' as const : parsed.ok ? 'custom' as const : 'standard' as const };
      });
      return nextGlassBuilderDraft({ ...current, sidelightType: value, sidelightSpecifications: specifications }, 'sidelightSpecifications', specifications);
    });
    setMessage('');
  }
  ...
  function commitSidelightWidth(identity: SidelightIdentity, value: string, panelMode: SidelightSpecification['panelSizeMode']) {
    ...
    const patch = type === 'Panel' && panelMode === 'standard' && !standard ? { panelSizeMode: 'custom' as const } : {};
    commitSpecificationDimension(identity, patch, { kind: 'sidelightWidth', ...identity, value });
  }
    ...
    <div aria-labelledby="glass-builder-title" aria-modal="true" className="glass-unit-builder m-auto grid h-full max-h-[96vh] w-full max-w-[min(1500px,98vw)] overflow-hidden bg-white shadow-2xl dark:bg-slate-900 sm:rounded-2xl" ref={dialog} role="dialog" tabIndex={-1}>
      ...
      <div className="grid min-h-0 overflow-y-auto p-3 lg:grid-cols-2 lg:gap-4">
        ...
        <section className="mt-5 grid content-start gap-3 lg:mt-0">
          ...
          <div className="rounded-xl border border-slate-200 p-3 dark:border-slate-700"><strong>Status: {calculation.status}</strong>{calculation.glassCalc ? <><dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1 text-sm" aria-label="Calculated measurements">{(calculation.glassCalc.resolvedSidelights as Array<{side:string;index:number;finishedWidth:string}> | undefined)?.map((entry) => <div key={`${entry.side}:${entry.index}`}><dt className="font-semibold">{entry.side === 'left' ? 'Left' : 'Right'} sidelight {entry.index} width</dt><dd>{entry.finishedWidth}</dd></div>)}{calculation.glassUnits.map((unit) => <div key={`${unit.position}:height`}><dt className="font-semibold">{unit.position} opening</dt><dd>{unit.width} × {unit.height}</dd></div>)}{calculation.glassCalc.transomWidth ? <div><dt className="font-semibold">Transom size</dt><dd>{String(calculation.glassCalc.transomWidth)} × {String(calculation.glassCalc.transomHeight)}</dd></div> : null}</dl><pre className="mt-2 whitespace-pre-wrap text-xs">{calculation.workorderDetail}</pre></> : null}</div>

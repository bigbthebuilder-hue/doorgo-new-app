  ...
  return (
    <figure className={`glass-unit-diagram ${compact ? 'glass-unit-diagram--compact' : ''}`} data-config={line.config} data-sidelight-type={type}>
      <svg aria-label={`${line.config} ${type} door-unit diagram`} preserveAspectRatio="xMidYMid meet" role="img" viewBox={`${-pad} ${-pad} ${layout.width + pad * 2} ${layout.height + pad * 2}`}>
        ...
        <defs>{layout.parts.filter((part) => part.label).map((part) => <clipPath id={`diagram-label-${diagramId}-${part.id}`} key={part.id}><rect height={part.height} width={part.width} x={part.x} y={part.y}/></clipPath>)}</defs>
        {layout.parts.map((part) => <g data-height={part.height} data-kind={part.kind} data-width={part.width} data-x={part.x} data-y={part.y} key={part.id}>
          ...
          {part.label ? <text clipPath={`url(#diagram-label-${diagramId}-${part.id})`} fill="var(--glass-diagram-stroke, rgb(15 23 42))" style={{ fontSize: Math.min(5, part.width / Math.max(2.5, part.label.length * .7), part.height / 5) }} x={part.x + part.width / 2} y={part.y + part.height / 2}>{part.label}</text> : null}
        </g>)}
      </svg>

  ...
  return (
    ...
    {inAppShell ? <ContextTopBar
      ...
      controls={<div className="app-job-context-fields app-job-shell-fields">
        ...
        <label className="app-job-context-field job-shell-fulfillment" htmlFor="fulfillmentPlan"><span>Fulfillment Plan</span><span className="job-shell-fulfillment-controls"><select disabled={!canEdit} id="fulfillmentPlan" onChange={(event) => update('fulfillmentPlan', event.target.value)} value={values.fulfillmentPlan}><option value="">Not selected</option><option value="Delivery">Delivery</option><option value="Customer Pickup">Customer Pickup</option></select>{values.fulfillmentPlan === 'Delivery' ? <DateOnlyPicker ariaLabel="Delivery Date" disabled={!canEdit} id="deliveryDate" onChange={(value)=>update('deliveryDate',value)} value={values.deliveryDate}/> : null}{values.fulfillmentPlan === 'Customer Pickup' ? <DateOnlyPicker ariaLabel="Customer Pickup Date" disabled={!canEdit} id="customerPickupDate" onChange={(value)=>update('customerPickupDate',value)} value={values.customerPickupDate}/> : null}</span></label>
        <label className="app-job-context-field job-shell-shop-date" htmlFor="shopDate"><span>Shop Date</span><DateOnlyPicker ariaLabel="Shop Date" disabled={!canEdit} id="shopDate" onChange={(value)=>update('shopDate',value)} value={values.shopDate}/></label>
    ...
    <JobEditorSurface inAppShell={inAppShell}>
      ...
      <div className="job-operational-strip mt-1 grid gap-1.5">
        ...
        {!inAppShell ? <section className="job-production-strip rounded-md border border-slate-200 p-1.5 dark:border-slate-700" aria-label="Production Setup"><div className="job-production-fields grid gap-1.5 md:grid-cols-3 xl:grid-cols-5"><Field error={fieldErrors.shopHours} label={`Shop Hours${effectiveShopHoursSource ? ` · ${effectiveShopHoursSource}` : ''}`} name="shopHours"><input className={inputClass} disabled={!canEdit} id="shopHours" min="0" onChange={(event) => update('shopHours', event.target.value)} step="0.25" type="number" value={effectiveShopHours}/></Field><Field label="Fulfillment Plan" name="fulfillmentPlan"><select className={inputClass} disabled={!canEdit} id="fulfillmentPlan" onChange={(event) => update('fulfillmentPlan', event.target.value)} value={values.fulfillmentPlan}><option value="">Not selected</option><option value="Delivery">Delivery</option><option value="Customer Pickup">Customer Pickup</option></select></Field>{values.fulfillmentPlan === 'Delivery' ? <Field label="Delivery Date" name="deliveryDate"><DateOnlyPicker ariaLabel="Delivery Date" className={inputClass} disabled={!canEdit} id="deliveryDate" onChange={(value)=>update('deliveryDate',value)} value={values.deliveryDate}/></Field> : null}{values.fulfillmentPlan === 'Customer Pickup' ? <Field label="Customer Pickup Date" name="customerPickupDate"><DateOnlyPicker ariaLabel="Customer Pickup Date" className={inputClass} disabled={!canEdit} id="customerPickupDate" onChange={(value)=>update('customerPickupDate',value)} value={values.customerPickupDate}/></Field> : null}<Field label="Shop Date" name="shopDate"><DateOnlyPicker ariaLabel="Shop Date" className={inputClass} disabled={!canEdit} id="shopDate" onChange={(value)=>update('shopDate',value)} value={values.shopDate}/></Field></div></section> : null}

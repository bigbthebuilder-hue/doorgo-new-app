...
export function LegacyJobImportReview({ defaultSalesperson }: { defaultSalesperson: string }) {
  ...
  if (review) {
    ...
    return <JobHeaderForm canEdit defaultSalesperson={defaultSalesperson} initialDraft={{ header: result.editor.header, lines: result.editor.lines }} initialJob={null} transferReview={context}/>;
  }

  return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900 sm:p-6">
    <h1 className="text-2xl font-semibold">Import Legacy Job</h1>
    <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">Select one downloaded DoorGo legacy-transfer JSON file. Reviewing or cancelling creates no native job.</p>

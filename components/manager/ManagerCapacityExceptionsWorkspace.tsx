...
function SpecialDayEditor({ canEdit, record, roster, onDone }: { canEdit: boolean; record: SpecialDayRecord | null; roster: CapacityStaff[]; onDone: () => void }) {
  ...
  const candidates = useMemo(() => date ? configuredStaffForDate(roster, date, existingIds) : [], [date, existingIds, roster]);
  ...
  return <form action={action} className="manager-editor" key={record?.specialDayId ?? 'new-special'}><input name="specialDayId" type="hidden" value={record?.specialDayId ?? ''}/><input name="revision" type="hidden" value={record?.revision ?? ''}/><div className="manager-form-row"><label>Date<input disabled={!canEdit} name="date" onChange={event => setDate(event.target.value)} required type="date" value={date}/></label><label>Title<input disabled={!canEdit} name="title" required defaultValue={record?.title}/></label><label>Details<input disabled={!canEdit} name="details" defaultValue={record?.details ?? ''}/></label></div>{date ? <fieldset className="manager-special-staff" disabled={!canEdit}><legend>Staff Working That Date</legend>{candidates.map(staff => <label key={staff.staffId}><input checked={selectedIds.has(staff.staffId)} name="staffId" onChange={() => toggleStaff(staff.staffId)} type="checkbox" value={staff.staffId}/><strong>{staff.displayName}</strong><span>{staff.capacityRole === 'direct_production' ? `${staff.productiveHours}h production` : 'Support'}</span></label>)}</fieldset> : <p className="manager-special-date-prompt">Select a date to load staff capacity.</p>}<output className="manager-readonly-capacity" aria-live="polite">Calculated Production capacity: {calculatedCapacity}h</output><p className="manager-capacity-hint">For a different final capacity, use Capacity Overrides.</p><footer><button disabled={!canEdit || pending}>{pending ? 'Saving…' : record ? 'Save Special Day' : 'Add Special Day'}</button>{record ? <button disabled={removing} formAction={remove}>Delete Special Day</button> : null}</footer><Result state={state.message ? state : removeState}/></form>;
}

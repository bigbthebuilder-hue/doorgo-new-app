...
| RO Width | `roWInput`; `roWidth` | `RO Width` | `ro_width` | dimension string | Required for glass configurations; blank for PKT/B.P.; strict blockers in §6. B.P. has no Finished Opening width input or calculation: F.O. width is Not Applicable and must not be requested, persisted, calculated or printed. | text/null. |
| RO Height | `roHInput`; `roHeight` | `RO Height` | `ro_height` | dimension string | Required for transom and optional for ordinary/custom cut-down. For B.P. this is optional entered Finished Opening height. Blank is valid and resolves to applicable finished slab height plus 2-3/4 inches; entered valid height is preserved and produces required slab height `FO − 2-3/4 inches`. Invalid/malformed/nonpositive input, or a result at or below zero, is blocked. | text/null. |
...
| B.P. | Interior bypass; no jamb/hinge/swing and no F.O. width. F.O. Height is optional. Entered height is preserved and required finished slab height is `FO − 2-3/4 inches`; blank resolves to applicable finished slab height plus 2-3/4 inches and represents no additional cut. Always show resolved F.O. Height in saved line summary, preview/detail and work order. Prep None or Half drill. | Approved native correction of `isBypassConfig`, `buildLine`, `generatedDetailBlock_`; deployed omission of full-height resolved F.O. display must not be reproduced. |
...
- B.P. Finished Opening width is explicitly Not Applicable. Its absence is never incomplete or blocking, and the calculator must not request, persist, derive or print it.
- B.P. Finished Opening height is optional. With an entered valid height, preserve it and calculate required finished door/slab height as `F.O. height − 2-3/4 inches`. With no entered height, resolve F.O. height as the applicable authoritative finished door/slab height plus 2-3/4 inches; this is the normal no-additional-cut state and remains Complete.
- The applicable B.P. door height is the selected standard finished height when no custom height applies, the validated custom finished slab height when applicable, or an existing authoritative cut-down result when approved line rules require it. No separate B.P. width rule may be invented.
- Blank B.P. height is valid. Invalid, malformed, zero or negative entered height is Blocked, as is a valid entered height whose 2-3/4-inch deduction produces a nonpositive door height. It is not an Incomplete condition.
...
- B.P. saved line summaries, line preview/details and work-order detail rows always show the resolved `F.O. Height` in canonical inches-only shop formatting. Example: an `80"` applicable door height with no entered F.O. resolves to `F.O. Height 82 3/4"`; entered `F.O. Height 80"` produces required door height `77 1/4"`. B.P. F.O. width is never shown.
...
- J3C Send uses the separately approved rules below. Both `jobs = view` and `jobs = use` may send the last successfully saved work order; `jobs = none` may not. Manager status provides no fallback and no separate `documents` permission applies.

### Approved J3C Send permission and safety boundary

- J3C sends only the existing generated work-order PDF. Glass measure sheets, picking tickets, other document types and sending from the main Jobs list are outside J3C.
- `jobs = view` and `jobs = use` may preview, download, print and send. `jobs = none` may do none of those operations. Manager status provides no fallback, and J3C does not use a separate `documents` permission.
- A user selects one or more active DoorGo users with logins. Arbitrary, manually entered, customer and external recipient addresses are prohibited. Customer Email remains contact data only and is never a recipient fallback.
- The browser submits stable recipient user IDs only. A dedicated narrowly scoped server-only directory module authenticates the requester through the normal server path, requires at least `jobs = view`, and only then may use the existing server-only Supabase service-role/Auth Admin capability to resolve active profiles and login emails. The server independently re-resolves every ID immediately before sending and rejects missing, unknown, inactive, duplicate or email-less users. Client-supplied email addresses are never authoritative.
- The service-role credential, Auth Admin client and unrestricted directory must never reach browser code. J3C adds no recipient table, schema migration or RLS change.
- Email delivery uses the official Resend Node.js package through a small replaceable server-only provider adapter. No provider call originates in browser code. Runtime delivery requires server-only `RESEND_API_KEY` and `DOORGO_EMAIL_FROM`; the visible sender name is `DoorGo`, the address must use a later configured verified company-owned domain, and no reply-to is required. Missing provider configuration fails cleanly and never reports false success.
- The fixed subject is `DoorGo Work Order – <Sales Order or DoorGo Reference>`. The fixed, non-editable body is `Please find document attached.` Neither value is accepted from the browser as authority.
- Send is generated wholly from the last successfully saved aggregate. Unsaved changes block Send and Send never auto-saves. The server reloads by immutable job ID, requires the expected saved revision, regenerates the J3A document and exact J3B PDF, rejects existing blockers and requires acknowledgement of existing non-blocking warnings.
- Missing or invalid recipients, insufficient access, stale revision, PDF failure or provider-configuration failure blocks Send. The server does not trust a browser-supplied aggregate, document model, PDF, filename, subject or body.
- Before sending, show selected recipient names, subject, attachment filename and existing warnings. The action label is `Send Email`. Deliberate resends are permitted.
- Enforce access, recipient, saved-revision and preflight checks at route/page where applicable, server action, recipient directory, send service, provider adapter and UI boundaries. Hiding the action is not authorization.
- Sending must not navigate away; modify the job, document, revision, lifecycle or identities; create or move production bookings; change fulfillment; mark paperwork complete; modify scheduling or Calendar data; or write hosted intake data.
- J3C is implemented locally, but no production provider credential, verified sender configuration, domain verification, hosted enablement or real email delivery is active or accepted. Manual acceptance remains pending.

### Approved J3C multi-recipient delivery and feedback

- Send one separate email per independently validated recipient so no recipient sees another recipient's email address. Attempt every validated recipient.
- All-success delivery shows the normal small success toast. No-success delivery shows the normal error toast. Partial delivery shows a concise result such as `Sent to 2 of 3 recipients. 1 failed.` The user remains on the current order for every outcome, and failed recipients may be deliberately resent.
- No user-facing send history, audit UI or substantial send-audit database subsystem is required. Minimal server/provider error logging and provider message identifiers may be retained for troubleshooting, but PDF contents, credentials and service-role values must never be logged.
...
- Use the same generated filename for preview/download when a filename is needed, browser printing/PDF saving where controllable and the PDF attached by J3C Send.
- The user-facing filename omits aggregate revision. J3C verifies the expected saved revision server-side and the attached PDF must correspond exactly to that revision.
...
- `jobs:view` and `jobs:use` may send the saved work-order PDF under the approved J3C contract. `jobs:none` and manager-only access may not send; no separate `documents` permission is introduced.
- Send permission, saved-revision, preflight and recipient checks must be repeated at every applicable server and UI boundary. The browser submits stable recipient user IDs only, and the server re-resolves them against active DoorGo profiles and Supabase Auth immediately before delivery.
- Recipient lookup is handled by a dedicated server-only module after normal requester authentication and authorization. Arbitrary, customer, external, inactive, unknown, duplicate and email-less recipients are rejected.
...
| **Resolved: B.P. Finished Opening rule** | F.O. width is Not Applicable. F.O. height is optional: entered height is preserved and yields required door height by subtracting 2-3/4 inches; blank height resolves from applicable authoritative door height plus 2-3/4 inches and remains valid. | **Approved.** Always show resolved F.O. Height in line summary, preview/detail and work order using inches-only formatting. Invalid/nonpositive entered height or nonpositive calculated door height is Blocked; blank is not Incomplete. |
...
| **Resolved: J2 permission semantics** | none=no access; view=read jobs and active/archived lines; use=J2 job/line edits and approved lifecycle transitions. | **Approved for J2.** J3 preview/print and J3C Send permissions are resolved separately below. |
...
| **Resolved and superseded: J3C Send permission and recipients** | Both `jobs=view` and `jobs=use` may send the saved work-order PDF to one or more active DoorGo login users; `jobs=none` and manager-only access may not. No separate `documents` permission applies. | **Approved.** This supersedes the former `jobs=use`-only test-send rule. Browser supplies stable IDs only; server re-resolves active profile and Auth email, rejects invalid recipients and never uses customer or arbitrary addresses. |
| **Resolved and superseded: J3C recipient source** | Use a dedicated narrowly scoped server-only directory after normal requester authentication and `jobs` authorization, then use existing server-only Supabase service-role/Auth Admin capability to resolve active login users. | **Approved.** This supersedes the local JSON allowlist. Never expose service-role/Auth Admin capability to the browser. No recipient table or schema migration is approved. |
| **Resolved and superseded: J3C provider and delivery** | Use a replaceable server-only Resend adapter, one separate message per recipient, the fixed subject/body and the exact regenerated J3B PDF. | **Implemented locally, not production-enabled.** This supersedes the local test outbox. The official dependency and server-only variable names are present; real credentials, verified sender configuration, hosted enablement and actual delivery require separate authorization and acceptance. Full, failed and partial outcomes use normal toasts and remain on the order. |
| **Resolved: J3 PDF filename** | `Work_Order_<SalesOrder>.pdf` when BizTrack Sales Order exists; otherwise `Work_Order_<DoorGoReference>.pdf`, using the same sanitized filename across preview/download, controllable browser PDF save and J3C attachment. | **Approved.** Sales Order suppresses DoorGo reference. Never include internal UUID, revision, customer/site/contact data. Replace unsafe/path/control/platform characters, collapse replacements where practical and fail safely without a valid visible identifier. J3C verifies the saved revision server-side without adding it to the filename. |
...
3. **J3 — work-order output and Send:** use the approved shared non-glass calculator, repository-saved aggregate, persisted J2 glass results, J3A document model and single J3B renderer. `jobs=view` and `jobs=use` may preview, download, print and send; `jobs=none` and manager-only access may not. J3C blocks dirty/stale/blocked output, revalidates stable recipient IDs against active DoorGo profiles and Supabase Auth in a dedicated server-only directory, and sends the regenerated PDF through a replaceable server-only Resend adapter as one separate message per recipient. Provider configuration and actual delivery remain separately authorized. PDF filenames retain sanitized visible-identifier precedence.

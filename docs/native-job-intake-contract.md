...
- Email delivery uses Resend through a small replaceable server-only provider adapter. No provider call originates in browser code. Later implementation may add the official Resend Node.js dependency and requires server-only `RESEND_API_KEY` and `DOORGO_EMAIL_FROM`; the visible sender name is `DoorGo`, the address must use a later configured verified company-owned domain, and no reply-to is required. Missing provider configuration fails cleanly and never reports false success.
...
- No production provider credential, verified sender configuration, domain verification or email delivery is authorized merely by documenting this contract. J3C remains unimplemented until a separate implementation task.
...
| **Resolved and superseded: J3C provider and delivery** | Use a replaceable server-only Resend adapter, one separate message per recipient, the fixed subject/body and the exact regenerated J3B PDF. | **Approved contract only.** This supersedes the local test outbox. Later implementation may add the official dependency and server-only `RESEND_API_KEY`/`DOORGO_EMAIL_FROM`; verified sender configuration and actual delivery require separate authorization. Full, failed and partial outcomes use normal toasts and remain on the order. |

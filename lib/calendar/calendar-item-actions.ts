...
import {loadCalendarNativeJobLinks} from '@/lib/production-board/native-job-links';
...
export type CalendarNoteEditInput={commandId:string;itemId:string;expectedRevision:number;title:string;details:string;linkedInternalJobId:string|null;salesperson:string;scheduledDate:string|null;boardStart:string;boardEndExclusive:string;weeks:number;today:string};
...
export async function searchScheduledCalendar(input:{query:string}):Promise<{ok:true;targets:CalendarSearchTarget[]}|{ok:false}>{
  ...
  try{const client=createTrustedReadOnlySupabaseClient();const pattern=`%${query.replaceAll('%','\\%').replaceAll('_','\\_')}%`;
    const [productionTitle,productionJob,itemCustomer,itemSales,itemTitle,itemDetails]=await Promise.all([
      ...
      client.from('dg_calendar_items').select('item_id,item_type,scheduled_date,linked_internal_job_id,customer_name,sales_order,title').ilike('details',pattern).not('scheduled_date','is',null).is('deleted_at',null).limit(20),
    ]);if([productionTitle,productionJob,itemCustomer,itemSales,itemTitle,itemDetails].some((result)=>result.error))return {ok:false};const portionMatches=await client.from('dg_fulfillment_order_portions').select('portion_id,sales_order').ilike('sales_order',pattern).limit(20);if(portionMatches.error)return {ok:false};const portionIds=(portionMatches.data??[]).map((row)=>row.portion_id);const membershipMatches=portionIds.length?await client.from('dg_calendar_item_orders').select('item_id,portion_id').in('portion_id',portionIds):{data:[],error:null};if(membershipMatches.error)return {ok:false};const matchedItemIds=Array.from(new Set((membershipMatches.data??[]).map((row)=>row.item_id)));const includedItemMatches=matchedItemIds.length?await client.from('dg_calendar_items').select('item_id,item_type,scheduled_date,linked_internal_job_id,customer_name,sales_order,title').in('item_id',matchedItemIds).not('scheduled_date','is',null).is('deleted_at',null):{data:[],error:null};if(includedItemMatches.error)return {ok:false};const targets=new Map<string,CalendarSearchTarget>();
    const productionRows=[...(productionTitle.data??[]),...(productionJob.data??[])];const itemRows=[...(itemCustomer.data??[]),...(itemSales.data??[]),...(itemTitle.data??[]),...(itemDetails.data??[]),...(includedItemMatches.data??[])];
    const visibleIds=Array.from(new Set(productionRows.map((row)=>row.job_id).filter((value):value is string=>Boolean(value))));const internalIds=Array.from(new Set(itemRows.map((row)=>row.linked_internal_job_id).filter((value):value is string=>Boolean(value))));const nativeLinks=getPermissionAccess(access,'jobs')!=='none'?await loadCalendarNativeJobLinks(visibleIds,internalIds,createJobIntakeRepository()):{byVisibleIdentifier:new Map(),byInternalJobId:new Map()};
    for(const row of productionRows)if(row.production_date){const native=row.job_id?nativeLinks.byVisibleIdentifier.get(row.job_id):undefined;targets.set(row.booking_id,{bookingId:row.booking_id,productionDate:row.production_date,customer:native?.customer?.trim()||row.title||'Untitled',jobId:native?.salesOrder?.trim()||row.job_id,calendarItemType:'production'});}
    for(const row of itemRows)if(row.scheduled_date){const native=row.linked_internal_job_id?nativeLinks.byInternalJobId.get(row.linked_internal_job_id):undefined;targets.set(`item:${row.item_id}`,{bookingId:`item:${row.item_id}`,productionDate:row.scheduled_date,customer:native?.customer?.trim()||row.customer_name||row.title||'Untitled',jobId:row.sales_order?.trim()||native?.salesOrder?.trim()||null,calendarItemType:row.item_type});}
...
export async function updateCalendarNote(input:CalendarNoteEditInput):Promise<CalendarCreateResult>{
  const result=await rpc('update_calendar_note',{p_command_id:input.commandId,p_item_id:input.itemId,p_expected_revision:input.expectedRevision,p_title:input.title,p_details:input.details,p_linked_internal_job_id:input.linkedInternalJobId,p_salesperson:input.salesperson,p_scheduled_date:input.scheduledDate});if(!result.ok)return result;
  const access=await getCurrentDoorGoAccess();const board=await loadProductionBoardReadOnly({boardStart:input.boardStart,boardEndExclusive:input.boardEndExclusive,weeks:input.weeks,today:input.today,includeNativeJobLinks:getPermissionAccess(access,'jobs')!=='none',includeOperationalCalendarItems:true});
  const card=[...board.needsAttentionCards,...board.days.flatMap((day)=>day.cards)].find((value)=>value.bookingId===`item:${input.itemId}`);return card?{ok:true,data:result.data,card}:{ok:false,code:'authoritative_read_failed',message:'The Note was saved but could not be reloaded. Reopen Calendar to reconcile it.'};
}

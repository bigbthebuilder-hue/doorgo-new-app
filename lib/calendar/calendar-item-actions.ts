...
export async function searchScheduledCalendar(input:{query:string}):Promise<{ok:true;targets:CalendarSearchTarget[]}|{ok:false}>{
  ...
  try{const client=createTrustedReadOnlySupabaseClient();const pattern=`%${query.replaceAll('%','\\%').replaceAll('_','\\_')}%`;
    const [productionTitle,productionJob,itemCustomer,itemSales,itemTitle]=await Promise.all([
      ...
      client.from('dg_calendar_items').select('item_id,item_type,scheduled_date,customer_name,sales_order,title').ilike('customer_name',pattern).not('scheduled_date','is',null).is('deleted_at',null).limit(20),
      client.from('dg_calendar_items').select('item_id,item_type,scheduled_date,customer_name,sales_order,title').ilike('sales_order',pattern).not('scheduled_date','is',null).is('deleted_at',null).limit(20),
      client.from('dg_calendar_items').select('item_id,item_type,scheduled_date,customer_name,sales_order,title').ilike('title',pattern).not('scheduled_date','is',null).is('deleted_at',null).limit(20),
    ]);if([productionTitle,productionJob,itemCustomer,itemSales,itemTitle].some((result)=>result.error))return {ok:false};const portionMatches=await client.from('dg_fulfillment_order_portions').select('portion_id,sales_order').ilike('sales_order',pattern).limit(20);if(portionMatches.error)return {ok:false};const portionIds=(portionMatches.data??[]).map((row)=>row.portion_id);const membershipMatches=portionIds.length?await client.from('dg_calendar_item_orders').select('item_id,portion_id').in('portion_id',portionIds):{data:[],error:null};if(membershipMatches.error)return {ok:false};const matchedItemIds=Array.from(new Set((membershipMatches.data??[]).map((row)=>row.item_id)));const includedItemMatches=matchedItemIds.length?await client.from('dg_calendar_items').select('item_id,item_type,scheduled_date,customer_name,sales_order,title').in('item_id',matchedItemIds).not('scheduled_date','is',null).is('deleted_at',null):{data:[],error:null};if(includedItemMatches.error)return {ok:false};const targets=new Map<string,CalendarSearchTarget>();
    for(const row of [...(productionTitle.data??[]),...(productionJob.data??[])])if(row.production_date)targets.set(row.booking_id,{bookingId:row.booking_id,productionDate:row.production_date,customer:row.title??'Untitled',jobId:row.job_id,calendarItemType:'production'});
    for(const row of [...(itemCustomer.data??[]),...(itemSales.data??[]),...(itemTitle.data??[])])if(row.scheduled_date)targets.set(`item:${row.item_id}`,{bookingId:`item:${row.item_id}`,productionDate:row.scheduled_date,customer:row.customer_name||row.title||'Untitled',jobId:row.sales_order,calendarItemType:row.item_type});
    for(const row of includedItemMatches.data??[])if(row.scheduled_date)targets.set(`item:${row.item_id}`,{bookingId:`item:${row.item_id}`,productionDate:row.scheduled_date,customer:row.customer_name||row.title||'Untitled',jobId:row.sales_order,calendarItemType:row.item_type});

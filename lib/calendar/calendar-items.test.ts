...
import {calendarItemCard,calendarItemLayerKey,calendarRecordKey,mergeCalendarItems,type CalendarItemRow} from './calendar-items';
import type {ProductionBoardViewModel} from '../production-board/types';

const row=(type:CalendarItemRow['item_type'],date:string|null):CalendarItemRow=>({item_id:'11111111-1111-4111-8111-111111111111',item_type:type,scheduled_date:date,linked_internal_job_id:null,customer_name:'Hamilton',sales_order:'123455',salesperson:'Alex',timing:'AM',fulfillment_note:'Call first',title:type==='note'?'Check measurements':null,details:null,day_order:2048,completed_at:null,revision:1});
...
const merged=mergeCalendarItems(board,[linked,delivery]);assert.equal(merged.days[0].cards.length,1);assert.equal(merged.needsAttentionCards.length,1);assert.equal(merged.days[0].totalKnownShopHours,4,'fulfillment has no Production capacity effect');

...
export type CalendarItemRow = {
  ...
  completed_at:string|null; revision:number|string;
};

export function calendarItemCard(row:CalendarItemRow, linked?:{internalJobId:string;customer:string|null;salesOrder:string|null;salesperson:string|null}):ProductionBoardCard {
  ...
  return {bookingId:`item:${row.item_id}`,recordKind:'calendar_item',calendarItemType:row.item_type,revision:Number(row.revision),type:linked?'doorgo_linked':'biztrack_only',
    ...
    locked:false,completedAt:row.completed_at,timing:row.timing,fulfillmentNote:row.fulfillment_note,details:row.details};
}

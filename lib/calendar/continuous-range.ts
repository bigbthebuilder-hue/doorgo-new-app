...
export function mergeContinuousCalendarBoards(current:ProductionBoardViewModel,incoming:ProductionBoardViewModel):ProductionBoardViewModel{
  ...
  return {...current,startDate,endDateExclusive,visibleWeekdayEndExclusive:addDaysToDateOnly(endDateExclusive,-2),weeks:weekCount(startDate,endDateExclusive),days:orderedDays,weekGroups:orderedWeeks,needsAttentionCards:dedupeCards(incoming.needsAttentionCards),calculationStartDate:minDate(current.calculationStartDate,incoming.calculationStartDate),staffAwayRoster:mergeRoster(current.staffAwayRoster??[],incoming.staffAwayRoster??[])};
}
const dedupeCards=(cards:ProductionBoardViewModel['needsAttentionCards'])=>[...new Map(cards.map((card)=>[card.bookingId,card])).values()];
const mergeRoster=(left:NonNullable<ProductionBoardViewModel['staffAwayRoster']>,right:NonNullable<ProductionBoardViewModel['staffAwayRoster']>)=>[...new Map([...left,...right].map((staff)=>[staff.staffId,staff])).values()].sort((a,b)=>a.displayName.localeCompare(b.displayName));

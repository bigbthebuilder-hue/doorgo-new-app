...
import { validateBackorderSalesOrder, type FulfillmentType } from './order-family';

type Failure={ok:false;code:string;message:string};
const fail=(code:string):Failure=>({ok:false,code,message:({wrong_order_family:'That Sales Order does not belong to this Job order family.',sales_order_exists:'That backorder Sales Order already exists.',jobs_permission_required:'Jobs use permission is required.',permission_required:'Calendar use permission is required.',stale_item:'This fulfillment appointment changed. Reopen it and try again.',unknown_order:'An Included Order is no longer available.',invalid_completion:'Select at least one included order that was fulfilled.'} as Record<string,string>)[code]??'Fulfillment could not be updated.'});
...
export async function addBackorder(input:{commandId:string;linkedInternalJobId:string;baseSalesOrder:string;backorderSalesOrder:string;itemType:FulfillmentType;scheduledDate:string|null;timing:string;fulfillmentNote:string;today:string}):Promise<{ok:true;card:ProductionBoardCard}|Failure>{
  ...
  const validation=validateBackorderSalesOrder(input.baseSalesOrder,input.backorderSalesOrder);if(!validation.ok)return {ok:false,code:'wrong_order_family',message:validation.message};
  const client=await createAuthenticatedSupabaseServerClient();const {data,error}=await client.rpc('add_fulfillment_backorder',{p_command_id:input.commandId,p_linked_internal_job_id:input.linkedInternalJobId,p_sales_order:validation.salesOrder,p_item_type:input.itemType,p_scheduled_date:input.scheduledDate,p_timing:input.timing,p_fulfillment_note:input.fulfillmentNote});
  ...
  const card=[...board.needsAttentionCards,...board.days.flatMap((day)=>day.cards)].find((item)=>item.bookingId===`item:${itemId}`);return card?{ok:true,card}:fail('unavailable');
}
...
export async function loadJobFulfillmentFamily(internalJobId:string):Promise<{ok:true;familyKey:string|null;orders:string[]}|Failure>{
  ...
  const {data,error}=await client.from('dg_fulfillment_order_portions').select('family_key,sales_order').eq('linked_internal_job_id',internalJobId).order('sales_order');if(error)return fail('unavailable');return {ok:true,familyKey:data?.[0]?.family_key??null,orders:(data??[]).map((row)=>row.sales_order)};
}

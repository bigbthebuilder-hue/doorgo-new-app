...
assert.deepEqual(calendarDayDropTarget(5,[]),{targetId:null,before:false},'empty day targets its bottom');
const dropRects=[{id:'a',top:10,bottom:20},{id:'b',top:24,bottom:34}];
assert.deepEqual(calendarDayDropTarget(22,dropRects),{targetId:'b',before:true},'gap preserves precise insertion');
assert.deepEqual(calendarDayDropTarget(40,dropRects),{targetId:null,before:false},'space below the final collapsed or expanded card targets the day bottom');

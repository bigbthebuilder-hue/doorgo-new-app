-- DIRECT-DIMENSION GLASS NARROW ROLLBACK
-- DO NOT RUN AFTER ANY JOB LINE HAS STORED NEW-FIELD DATA
-- DO NOT USE AS A GENERAL NATIVE-JOB ROLLBACK
-- Before first use only; after first use requires a data-preserving rollback.
BEGIN;
SET LOCAL lock_timeout='5s'; SET LOCAL statement_timeout='60s';
LOCK TABLE public.dg_native_job_lines IN ACCESS EXCLUSIVE MODE;
DO $preconditions$
BEGIN
 IF pg_catalog.to_regprocedure('public.dg_validate_direct_dimension_glass_source(jsonb)') IS NULL THEN RAISE EXCEPTION 'rollback.helper_missing'; END IF;
 IF EXISTS(SELECT 1 FROM (VALUES ('sidelight_specifications','jsonb',true),('transom_t_bar_size','text',false),('transom_glass_type_code','text',false),('transom_custom_glass_description','text',false)) e(n,t,nn) WHERE NOT EXISTS(SELECT 1 FROM pg_catalog.pg_attribute a JOIN pg_catalog.pg_class c ON c.oid=a.attrelid JOIN pg_catalog.pg_namespace ns ON ns.oid=c.relnamespace WHERE ns.nspname='public' AND c.relname='dg_native_job_lines' AND a.attname=e.n AND NOT a.attisdropped AND pg_catalog.format_type(a.atttypid,a.atttypmod)=e.t AND a.attnotnull=e.nn)) THEN RAISE EXCEPTION 'rollback.feature_column_gate_failed'; END IF;
 IF EXISTS(SELECT 1 FROM (VALUES ('dg_native_job_lines_sidelight_specifications_shape'),('dg_native_job_lines_sidelight_t_bar_sizes'),('dg_native_job_lines_sidelight_glass_codes'),('dg_native_job_lines_panel_size_modes'),('dg_native_job_lines_transom_t_bar_size'),('dg_native_job_lines_transom_glass_type_code')) e(n) WHERE NOT EXISTS(SELECT 1 FROM pg_catalog.pg_constraint k WHERE k.conrelid='public.dg_native_job_lines'::regclass AND k.conname=e.n)) THEN RAISE EXCEPTION 'rollback.constraint_gate_failed'; END IF;
 IF (SELECT count(*) FROM pg_catalog.pg_proc WHERE oid=pg_catalog.to_regprocedure('public.dg_create_native_job(uuid,text,text,text,jsonb,jsonb)'))<>1 OR (SELECT count(*) FROM pg_catalog.pg_proc WHERE oid=pg_catalog.to_regprocedure('public.dg_update_native_job(uuid,bigint,jsonb,jsonb)'))<>1 OR (SELECT count(*) FROM pg_catalog.pg_proc WHERE oid=pg_catalog.to_regprocedure('public.dg_create_transferred_native_job(uuid,jsonb,jsonb,jsonb)'))<>1 THEN RAISE EXCEPTION 'rollback.rpc_gate_failed'; END IF;
 IF EXISTS(SELECT 1 FROM (VALUES ('public.dg_create_native_job(uuid,text,text,text,jsonb,jsonb)'::text),('public.dg_update_native_job(uuid,bigint,jsonb,jsonb)'),('public.dg_create_transferred_native_job(uuid,jsonb,jsonb,jsonb)')) e(s) JOIN pg_catalog.pg_proc p ON p.oid=pg_catalog.to_regprocedure(e.s) WHERE pg_catalog.pg_get_functiondef(p.oid) NOT LIKE '%sidelight_specifications%' OR pg_catalog.pg_get_functiondef(p.oid) NOT LIKE '%transom_t_bar_size%' OR pg_catalog.pg_get_functiondef(p.oid) NOT LIKE '%transom_glass_type_code%' OR pg_catalog.pg_get_functiondef(p.oid) NOT LIKE '%transom_custom_glass_description%' OR pg_catalog.pg_get_functiondef(p.oid) NOT LIKE '%dg_validate_direct_dimension_glass_source%') THEN RAISE EXCEPTION 'rollback.rpc_shape_failed'; END IF;
 IF EXISTS(SELECT 1 FROM public.dg_native_job_lines WHERE sidelight_specifications IS DISTINCT FROM '[]'::jsonb OR transom_t_bar_size IS NOT NULL OR transom_glass_type_code IS NOT NULL OR transom_custom_glass_description IS NOT NULL) THEN RAISE EXCEPTION 'rollback.accepted_data_exists'; END IF;
END;$preconditions$;
...
ALTER FUNCTION public.dg_create_native_job(uuid,text,text,text,jsonb,jsonb) SECURITY DEFINER;
ALTER FUNCTION public.dg_create_native_job(uuid,text,text,text,jsonb,jsonb) SET search_path='';
...
CREATE OR REPLACE FUNCTION public.dg_create_transferred_native_job(p_command_id uuid, p_provenance jsonb, p_header jsonb, p_lines jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
ALTER FUNCTION public.dg_create_transferred_native_job(uuid,jsonb,jsonb,jsonb) SECURITY DEFINER;
ALTER FUNCTION public.dg_create_transferred_native_job(uuid,jsonb,jsonb,jsonb) SET search_path='';
...
ALTER TABLE public.dg_native_job_lines DROP CONSTRAINT dg_native_job_lines_sidelight_specifications_shape,DROP CONSTRAINT dg_native_job_lines_sidelight_t_bar_sizes,DROP CONSTRAINT dg_native_job_lines_sidelight_glass_codes,DROP CONSTRAINT dg_native_job_lines_panel_size_modes,DROP CONSTRAINT dg_native_job_lines_transom_t_bar_size,DROP CONSTRAINT dg_native_job_lines_transom_glass_type_code;
ALTER TABLE public.dg_native_job_lines DROP COLUMN sidelight_specifications,DROP COLUMN transom_t_bar_size,DROP COLUMN transom_glass_type_code,DROP COLUMN transom_custom_glass_description;
DROP FUNCTION public.dg_validate_direct_dimension_glass_source(jsonb);
DO $postconditions$ BEGIN IF pg_catalog.to_regprocedure('public.dg_validate_direct_dimension_glass_source(jsonb)') IS NOT NULL THEN RAISE EXCEPTION 'rollback.helper_removal_failed'; END IF; IF pg_catalog.to_regclass('public.dg_native_jobs') IS NULL OR pg_catalog.to_regclass('public.dg_native_job_lines') IS NULL OR pg_catalog.to_regclass('public.dg_native_job_reference_seq') IS NULL THEN RAISE EXCEPTION 'rollback.preserved_object_failed'; END IF; END;$postconditions$;
COMMIT;
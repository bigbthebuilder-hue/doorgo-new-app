-- DIRECT-DIMENSION UPDATE-RPC ALLOWLIST NARROW ROLLBACK
-- RESTORES ONLY THE PRE-CORRECTION UPDATE RPC
...
DO $post$ BEGIN IF (SELECT pg_catalog.md5(pg_catalog.pg_get_functiondef(oid)) FROM pg_catalog.pg_proc WHERE oid=pg_catalog.to_regprocedure('public.dg_update_native_job(uuid,bigint,jsonb,jsonb)'))<>'6819aa940c8e894c23601b73e870fd28' THEN RAISE EXCEPTION 'rollback.restore_md5_mismatch'; END IF; END; $post$;
COMMIT;

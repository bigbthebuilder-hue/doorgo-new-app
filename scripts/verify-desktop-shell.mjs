...
assert.match(nav, /label:'Production Board'/);
assert.doesNotMatch(nav, /label: 'Edit Schedule'/, 'Legacy Edit Schedule must not appear in normal staff navigation');
assert.doesNotMatch(nav, /label: 'Past Schedule'|label: 'Carry Checkpoint'/, 'Legacy utility labels must not consume permanent rail positions');

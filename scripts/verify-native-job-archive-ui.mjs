...
assert.match(control, /aria-modal="true"/, 'archive confirmation must use an accessible modal dialog');
assert.match(control, />Cancel<\/button>/, 'archive dialog must provide cancellation');
assert.doesNotMatch(control, /window\.confirm/, 'archive interaction must not use a browser confirm prompt');

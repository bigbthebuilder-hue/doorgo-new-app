...
const validateUpdateRollback = (sql) => { const code=stripComments(sql);const lower=code.toLowerCase();const definition=updateDefinition(code);assert.match(lower,/^\s*begin;[\s\S]*be3117f9494d85c82adb2359bf2040d1[\s\S]*6819aa940c8e894c23601b73e870fd28[\s\S]*commit;\s*$/);assert.equal((lower.match(/create or replace function public\.dg_[a-z0-9_]+/g)??[]).length,1);assert.ok(definition);assert.doesNotMatch(lower.replace(definition.toLowerCase(),''),/rollback-direct-dimension-glass-sources|rollback-native-job-persistence|\b(?:alter table|drop column|drop constraint|nextval|setval|alter sequence|insert into|delete from)\b/);};
...
const validateGlassAcceptance=(sql)=>{const x=sql.toLowerCase();assert.match(x,/^-- rollback-contained direct-dimension glass rpc acceptance[\s\S]*\bbegin;[\s\S]*\brollback;[\s\S]*\bselect\b/);assert.doesNotMatch(x,/\bcommit;|pg_catalog\.nextval|\bsetval\b|alter sequence|restart sequence|dg_create_native_job\([^;]*,'native'|dg_archive_native_job|supabase_migrations|dg_production|dg_calendar|dg_fulfillment|dg_email/);for(const a of ['dg_create_native_job(','dg_update_native_job(','dg_create_transferred_native_job(','stale_revision','idempotency_conflict','sequence_unchanged','overall_acceptance_passed'])assert.ok(x.includes(a),`Missing acceptance evidence ${a}`);for(const r of ['unsupported_t_bar','numeric_t_bar','duplicate_identity','missing_side','string_index','fractional_index','outside_index','unsupported_width','blank_width','unknown_glass','custom_without_description','invalid_description_scalar','unsupported_panel_mode','unexpected_key','non_array','invalid_transom_tbar','invalid_transom_glass','malformed_native','malformed_transfer'])assert.ok(x.includes(r),`Missing rejection ${r}`);for(const field of directFields)assert.ok(x.includes(field),`Acceptance must exercise ${field}`);assert.match(x,/dg_update_native_job\(v_transfer_job[\s\S]{0,900}'transom_t_bar_size'[\s\S]{0,900}'transom_glass_type_code'[\s\S]{0,900}'transom_custom_glass_description'/);for(const field of ['origin','legacy_job_id','legacy_identifier_kind','visible_identifier','visible_identifier_kind','transfer_source_system','transfer_schema','transfer_version','transfer_source_identifier_kind','transfer_source_identifier_value','transfer_source_saved_at','transfer_exported_at','transfer_source_fingerprint'])assert.ok((x.match(new RegExp(`'${field}'`,'g'))??[]).length>=2,`Acceptance must snapshot and compare ${field}`);assert.match(x,/v_provenance_after is distinct from v_provenance_snapshot/);assert.match(x,/line_status','archived'[\s\S]*validation_failed[\s\S]*archived_transferred_line_not_rejected/);assert.doesNotMatch(x,/raise exception 'malformed_(?:native|transfer)_not_rejected'[\s\S]{0,100}exception when others/);assert.match(x,/v_rejection_observed:=false;[\s\S]*get stacked diagnostics v_error=message_text;[\s\S]*if v_error like '%validation_failed%'[\s\S]*if not v_rejection_observed/);for(const field of ['fixture_creation_passed','all_four_update_fields_persisted','all_four_update_fields_returned_correctly','revision_increment_passed','stale_revision_rejected','archive_merge_checks_passed','rollback_containment_passed','sequence_unchanged','overall_acceptance_passed'])assert.ok(x.includes(field),'Missing final acceptance field '+field);assert.match(x,/dg_create_transferred_native_job[\s\S]*dg_update_native_job/);assert.doesNotMatch(x,/v_result:=public\.dg_create_native_job/);assert.match(x,/v_actor_count<>1/);assert.match(x,/v_after_sequence<>v_seq[\s\S]*v_after_called is distinct from v_seq_called/);assert.doesNotMatch(x,/dg-000013|job-0065/)};validateGlassAcceptance(glassRpcAcceptance);for(const bad of [glassRpcAcceptance.replace('ROLLBACK;','COMMIT;'),glassRpcAcceptance.replace('ROLLBACK;',''),glassRpcAcceptance.replace("'legacy_transfer'","'native'"),glassRpcAcceptance.replace('v_after_sequence<>v_seq','false'),glassRpcAcceptance.replaceAll('public.dg_update_native_job','public.missing_update'),glassRpcAcceptance.replace('unsupported_t_bar','rejection_removed'),glassRpcAcceptance.replace("'line_status','Archived'","'line_status','Active'")])assert.throws(()=>validateGlassAcceptance(bad));
assert.ok(glassRpcAcceptance.includes("v_header jsonb:=pg_catalog.jsonb_build_object('biztrack_sales_order','NONPROD-DDG-TRANSFER-980001'"),'Acceptance header must carry the synthetic Sales Order');
assert.match(glassRpcAcceptance,/'source_identifier_kind','biztrack_sales_order','source_identifier_value','NONPROD-DDG-TRANSFER-980001'/,'Transfer provenance must use the matching synthetic Sales Order');
assert.doesNotMatch(glassRpcAcceptance,/'source_identifier_kind','legacy_job_id','source_identifier_value','NONPROD-/,'Synthetic non-JOB identifiers cannot claim legacy_job_id kind');

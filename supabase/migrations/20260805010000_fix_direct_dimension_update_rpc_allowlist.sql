...
DO $post$ BEGIN IF (SELECT pg_catalog.md5(pg_catalog.pg_get_functiondef(oid)) FROM pg_catalog.pg_proc WHERE oid=pg_catalog.to_regprocedure('public.dg_update_native_job(uuid,bigint,jsonb,jsonb)'))<>'1c080e0832feb2821df8248e715f0c96' THEN RAISE EXCEPTION 'correction.post_md5_mismatch'; END IF; END; $post$;
DO $post_contract$ BEGIN IF NOT EXISTS(SELECT 1 FROM pg_catalog.pg_proc p WHERE p.oid=pg_catalog.to_regprocedure('public.dg_update_native_job(uuid,bigint,jsonb,jsonb)') AND pg_catalog.pg_get_userbyid(p.proowner)='postgres' AND p.prosecdef AND p.proconfig=ARRAY['search_path=""']) OR (SELECT pg_catalog.count(*) FROM information_schema.routine_privileges WHERE specific_schema='public' AND routine_name='dg_update_native_job' AND grantee IN ('postgres','authenticated') AND privilege_type='EXECUTE')<>2 OR EXISTS(SELECT 1 FROM information_schema.routine_privileges WHERE specific_schema='public' AND routine_name='dg_update_native_job' AND grantee IN ('PUBLIC','anon','service_role') AND privilege_type='EXECUTE') OR NOT EXISTS(SELECT 1 FROM public.dg_native_job_reference_seq WHERE last_value=14 AND is_called=true) THEN RAISE EXCEPTION 'correction.post_contract_mismatch'; END IF; END; $post_contract$;
COMMIT;

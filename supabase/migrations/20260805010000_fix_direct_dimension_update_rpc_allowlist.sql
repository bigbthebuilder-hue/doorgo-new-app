...
DO $post$ BEGIN IF (SELECT pg_catalog.md5(pg_catalog.pg_get_functiondef(oid)) FROM pg_catalog.pg_proc WHERE oid=pg_catalog.to_regprocedure('public.dg_update_native_job(uuid,bigint,jsonb,jsonb)'))<>'be3117f9494d85c82adb2359bf2040d1' THEN RAISE EXCEPTION 'correction.post_md5_mismatch'; END IF; END; $post$;

-- Narrow correction: preserve transferred BizTrack identity during native-job update.
BEGIN;

DO $correction$
DECLARE
 v_start_md5 constant text := 'be3117f9494d85c82adb2359bf2040d1';
 v_expected_md5 constant text := '4b16dfb5896d1ea080edc05e419de6c2';
 ...
 v_before_definition text; v_after_definition text; v_candidate_definition text;
...
BEGIN
 ...
 IF v_before_md5 IS DISTINCT FROM v_start_md5 THEN RAISE EXCEPTION 'transferred_biztrack_update.start_md5_mismatch'; END IF;
 IF (pg_catalog.length(v_before_definition)-pg_catalog.length(pg_catalog.replace(v_before_definition,v_old_expression,'')))
      / pg_catalog.length(v_old_expression) <> 1
 THEN RAISE EXCEPTION 'transferred_biztrack_update.expected_single_expression_missing'; END IF;
 IF pg_catalog.strpos(v_before_definition,v_new_expression)<>0 THEN RAISE EXCEPTION 'transferred_biztrack_update.candidate_already_present'; END IF;

 v_candidate_definition:=pg_catalog.replace(v_before_definition,v_old_expression,v_new_expression);
 ...
 FROM pg_catalog.pg_proc AS p
 WHERE p.oid=pg_catalog.to_regprocedure('public.dg_update_native_job(uuid,bigint,jsonb,jsonb)');
 IF v_after_md5 IS DISTINCT FROM v_expected_md5 THEN RAISE EXCEPTION 'transferred_biztrack_update.post_md5_mismatch'; END IF;

 v_contract_passed:=pg_catalog.strpos(v_after_definition,v_new_expression)>0
  AND pg_catalog.strpos(v_after_definition,v_old_expression)=0
 ...
 IF NOT v_contract_passed THEN RAISE EXCEPTION 'transferred_biztrack_update.contract_mismatch'; END IF;
 ...
 THEN RAISE EXCEPTION 'transferred_biztrack_update.function_security_mismatch'; END IF;
 ...
 IF NOT v_grants_passed THEN RAISE EXCEPTION 'transferred_biztrack_update.grant_mismatch'; END IF;
 ...
 THEN RAISE EXCEPTION 'transferred_biztrack_update.sequence_changed'; END IF;
END;
$correction$;
